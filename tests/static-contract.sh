...
require_literal "$INSTALL" 'readonly SCHEDULE_HELPER_DST="/usr/local/sbin/codex-window-anchor-schedule"'
...
require_literal "$UNINSTALL" 'readonly SCHEDULE_HELPER_DST="/usr/local/sbin/codex-window-anchor-schedule"'
...
require_literal "$README" 'sudo codex-window-anchor-schedule \'
require_literal "$README" 'More scheduled Anchors do **not** create additional quota.'
reject_literal "$README" '08:00 Asia/Shanghai'
reject_literal "$README" '13:05 Asia/Shanghai'
reject_literal "$README" '18:10 Asia/Shanghai'
...
schedule_mode="$(git -C "$REPO_ROOT" ls-files -s -- scripts/configure-schedule.sh | awk '{ print $1 }')"
[[ "$schedule_mode" == "100755" ]] || \
  fail "scripts/configure-schedule.sh must be tracked with Git mode 100755 (current: ${schedule_mode:-untracked})"
