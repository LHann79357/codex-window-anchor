...
require_literal "$UNINSTALL" 'resolve_service_stop_state'
require_literal "$UNINSTALL" '--property=LoadState'
require_literal "$UNINSTALL" '--property=ActiveState'
require_literal "$UNINSTALL" '[[ "$load_state" == "not-found" && "$active_state" == "inactive" ]]'
require_literal "$UNINSTALL" '[[ "$load_state" == "loaded" && "$active_state" == "inactive" ]]'
require_literal "$UNINSTALL" 'if ! systemctl stop "$SERVICE_NAME"; then'
reject_literal "$UNINSTALL" 'systemctl stop "$SERVICE_NAME" || \'
...
reject_literal "$TIMER_TEMPLATE" 'OnCalendar='

require_literal "$README" '# Set your own timezone and run time'
require_literal "$README" 'automatic scheduling does not begin until you explicitly confirm and enable it.'
require_literal "$README" 'codex-window-anchor-schedule \'
require_literal "$README" '  --timezone Etc/UTC \'
require_literal "$README" '  --time 09:30'
require_literal "$README" 'performs bounded self-elevation through `/usr/bin/sudo` only when it needs to write the systemd Timer.'
require_literal "$README" 'does not increase Codex quota, create additional allowance, bypass limits, force quota resets'
